...
def download_stocks(companies, days_back_start, days_back, **kwargs):
    last_date = datetime.today() - timedelta(days=days_back_start)
...
def store_tables(date_table, stock_table):

    hook = PostgresHook(postgres_conn_id = 'postgres')

    with hook.get_conn() as conn:
        with conn.cursor() as cur:
            SQL_STATEMENT = f"COPY {date_table} FROM stdin WITH DELIMITER as ','"
            with open('/tmp/date.csv', 'r') as f:
                cur.copy_expert(SQL_STATEMENT, f)

            SQL_STATEMENT = f"COPY {stock_table} FROM stdin WITH DELIMITER as ','"
